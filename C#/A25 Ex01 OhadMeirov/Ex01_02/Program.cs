...
namespace Ex01_02
{
    public class Program
    {
        ...
        public static void TreeLetterPrinter(char[] i_LettersToPresent, int i_TimesLettersAppearInALine, int i_CurrentPlaceInTheArray, int i_SpacesToMove , int i_LastLetterPlacesToMove, int i_NumberOfTheRow)
        {
            ...
            if (i_LettersToPresent.Length == i_CurrentPlaceInTheArray + 1)
            {
                sbOneLineBeforeTheLastLine.Append(i_NumberOfTheRow).Append(' ', i_LastLetterPlacesToMove);
                sbOneLineBeforeTheLastLine.Append('|').Append(i_LettersToPresent[i_CurrentPlaceInTheArray]).Append('|').Append('\n');
                sbTheLastLine.Append(i_NumberOfTheRow + 1).Append(' ', i_LastLetterPlacesToMove);
                sbTheLastLine.Append('|').Append(i_LettersToPresent[i_CurrentPlaceInTheArray]).Append('|').Append('\n');
            ...
            {                
                if (flagToCheckOneTimeInTheLoop == true)
                {
                    ...
                    Console.Write(i_LettersToPresent[i_CurrentPlaceInTheArray + i] + " ");
            }
            Console.WriteLine();
            Console.WriteLine();
            TreeLetterPrinter(i_LettersToPresent, i_TimesLettersAppearInALine + 2, i_CurrentPlaceInTheArray + i_TimesLettersAppearInALine, i_SpacesToMove - 2, i_LastLetterPlacesToMove, i_NumberOfTheRow + 1);
        }              
    }
}
